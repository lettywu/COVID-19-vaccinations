...
*Alyssia Oh, Jesse Tao, Letty Wu, Rahul Parab*
...
The Primary Source for Covid Data comes from California Open Data Portal website. The following data sets were used:
...
[(Back to top)](#table-of-contents)
 ...
 There are two suppliers for COVID vaccine so far, Pfizer and Moderna, we acquired both data from Data.CDC.gov, since they are provided by HHS, both datasets have the same format. 
- For our project's purpose, we have to merge them together, we removed some islands given most of their data are NaNs, then removed the punctuations and converted them to numeric data for merging later. We only kept the first allocation data, since they are identical to the second allocation. After combining the vaccine data from Pfizer and Moderna, we also add in the population for each state. 
- We took the scraped cumulative vaccine distribution and administration data per state from CDC Data Tracker and took out only the data for California, and created extra columns to determine the daily change in vaccines administered.

Next we took a look at the mask use in each county, but the data was not really useful in helping us predict COVID-19 Cases in California. We moved on the normalizing our data to be based on a per 100k population, as many of the outbreak metrics are based on cases per 100k people. Below is a plot showing the 7 day rollowing average of newly confirmed cases per 100k people. 


![7dayrolling100k](./images/7_day_newly_confirmed_all_counties.png)

As analyzing all this data on separate plots is very difficult, we decided to use GeoPandas so we could visualize it in a map like below.

![maphospital](./images/covid_density_by_population.png)
...
(2.) **Extent of Data** The Vaccine Allocation / Distribution / Adminstration information existed only till 1/26/2021. As the predictions we made were beyond 1/26/2021, we used Time Series ( AutoReg ) to create an estimated dataset that predicted vaccine administration post 1/26/2021. This data has also been manually edited based on inputs from Press Release made by the President on number of Vaccines to be distributed till July 2021 to address the entire US population.
...
With more time, here are some further steps that could be taken to improve our model:

- Create an ETL pipeline that grabs new case data everyday and reruns our time series prediction models so vaccines can be allocated more dynamically.
- Get more data about transmission rates, lockdown protocols, more detailed demographics, and vaccine administration per county.
- Run grid searches on our neural network topology to find a more ideal model that can predict for large swings in COVID-19 cases.
- Figure out a way to feed more than one feature into our LSTM model to make it more robust and better at predicting future COVID-19 cases.
- Create a new COVID-19 vaccine distribution strategy that prioritizes giving vaccines to people that are more likely to spread the disease or are at higher risk of death, rather than just healthcare workers and people of old age.

# Additional Information And Citations
[(Back to top)](#table-of-contents)
We also built a web app with additional visualizations for our process that can be found [here.](https://covid.jesseptao.com)
